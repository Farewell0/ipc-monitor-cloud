...
public class PushStreamAuthCacheService {
    ...
    public void setPushToken(String token) {
        redisService.getRedisServiceApiForMonitor().set(PUSH_STREAM_AUTH_TOKEN_HEADER, token);
    }

    public void setPushToken(String token, long expires) {
        redisService.getRedisServiceApiForMonitor().set(PUSH_STREAM_AUTH_TOKEN_HEADER, token, expires);
    }

    public String getPushToken() {
        return redisService.getRedisServiceApiForMonitor().get(PUSH_STREAM_AUTH_TOKEN_HEADER, String.class);
    }
}
